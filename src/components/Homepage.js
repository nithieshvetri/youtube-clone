...
useEffect(()=>{
    fetchRequest({'searc':search?search:"",'next':next,'data':data?data:[]});
    setscroll(0)
...
const handleScroll = () => {
  ...
  React.useEffect(() => {
    ...
    return (
        <div>{
            data[0]?<Structure data={data} next={next} /> :<div className="loader"><img src={loader} alt="loader" /></div>}
        </div>
    )
}
