...
import {fetchRequest, Play,Scroll} from '../actions'
import { connect } from "react-redux";
import {GrChannel} from 'react-icons/gr'



function Media({data,play,next,Scroll,fetchrequest}) {
...
const navigate=useNavigate()
 ...
 function Structure({data,play,Scroll,next,fetchrequest}) {
  return (
    <Box sx={{ overflow: 'hidden' }}>
      <Media  data={data} next={next} Scroll={Scroll} fetchrequest={fetchrequest} play={play}/>
    </Box>
  );
}
...
const mapstatetodispatch=dispatch=>{
  return {
    ...
    Scroll:(next)=>dispatch(Scroll(next))
  }
}
